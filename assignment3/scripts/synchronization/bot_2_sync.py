...
def algo(yaw1,yaw2,yaw3,yaw4,L):
    ...
    y=np.sin(yaw2-np.array([yaw1,yaw2,yaw3,yaw4]))

    #w=0.4+np.sum(np.multiply(L[1,:],y))    # phase balancing
    #w=0-np.sum(np.multiply(L[1,:],y))    # phase sync: headings are synced same as consensus
    
    w=0.9-np.sum(np.multiply(L[1,:],y))  # freq sync: bots rotate with same w
