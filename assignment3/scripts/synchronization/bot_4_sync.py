...
def algo(yaw2,yaw3,yaw4,L):
    ...
    y=np.sin(yaw4-np.array([yaw1,yaw2,yaw3,yaw4]))

    #w=0.8+np.sum(np.multiply(L[3,:],y))    # phase balancing 
    #w=0-np.sum(np.multiply(L[3,:],y))    # phase sync: headings are synced same as consensus
    
    w=1.1-np.sum(np.multiply(L[3,:],y))  # freq sync: bots rotate with same w
...
if __name__ == "__main__":
    ...
    publish_to_cmd_vel = rospy.Publisher('bot_4/cmd_vel', Twist, queue_size = 10)
