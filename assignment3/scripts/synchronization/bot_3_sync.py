...
yaw4=0
L=np.zeros((4,4))
def algo(yaw2,yaw3,yaw4,L):
    #global yaw1,yaw2,yaw3,yaw4
    y=np.sin(yaw3-np.array([yaw1,yaw2,yaw3,yaw4]))

    #w=0.6+np.sum(np.multiply(L[2,:],y))   # phase balancing
    #w=0-np.sum(np.multiply(L[2,:],y))   # phase sync: headings are synced same as consensus
    
    w=1.2-np.sum(np.multiply(L[2,:],y)) # freq sync: bots rotate with same w
...
if __name__ == "__main__":
    ...
    rospy.init_node('bot_3')
    
    r=rospy.Rate(10)
    subscribe_to_L_A=rospy.Subscriber('laplacian_adjacency_matrix_sync', Float64MultiArray, callback = L_A)
    ...
    publish_to_cmd_vel = rospy.Publisher('bot_3/cmd_vel', Twist, queue_size = 10)
