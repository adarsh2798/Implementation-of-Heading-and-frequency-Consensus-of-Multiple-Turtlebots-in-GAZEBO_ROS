...
yaw4=0
L=np.zeros((4,4))
def algo(yaw1,yaw2,L):
    
    y=np.sin(yaw1-np.array([yaw1,yaw2,0,0]))
    
    

    w=0.8-np.sum(np.multiply(L[0,:],y)) # freq sync: bots rotate with same w
...
if __name__ == "__main__":
    ...
    publish_to_cmd_vel = rospy.Publisher('bot_1/cmd_vel', Twist, queue_size = 10)
